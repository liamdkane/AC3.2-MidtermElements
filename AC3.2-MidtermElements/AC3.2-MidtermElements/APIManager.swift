...
class APIRequestManager {
    ...
    func parseData (data: Data) -> [Element]? {
        var elements: [Element] = []
        
        do {
            let json = try JSONSerialization.jsonObject(with: data, options: [])
            guard let elementsArr = json as? [[String: Any]] else {
                print("Elements Arr failed")
                return nil }
            elements = elementsArr.flatMap{ Element(elementDict: $0) }
        }
        catch {
